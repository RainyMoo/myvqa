...
class Gate_MHAtt(nn.Module):
    def __init__(self, __C, base=64, hsize_k=None, bias=False):
        super(Gate_MHAtt, self).__init__()
        self.__C = __C
        self.HBASE = base
        ...
        self.linear_k = nn.Linear(__C.HSIZE, self.HSIZE_INSIDE, bias=bias)
        self.linear_q = nn.Linear(__C.HSIZE, self.HSIZE_INSIDE, bias=bias)
        self.linear_c1 = nn.Linear(__C.HSIZE, 1, bias=bias)
        self.linear_c2 = nn.Linear(__C.HSIZE, 1, bias=bias)
        self.linear_gk = nn.Linear(__C.HSIZE, 1, bias=bias)
        self.linear_gq = nn.Linear(__C.HSIZE, 1, bias=bias)
        self.linear_merge = nn.Linear(self.HSIZE_INSIDE, __C.HSIZE, bias=bias)
        self.dropout = nn.Dropout(__C.DROPOUT_R)
    ...
    def forward(self, v, k, q, c, mask=None):
        ...
        v = v.view(n_batches, -1, self.HHEAD, self.HBASE).transpose(1, 2)    
...
class GateSelfAtt(nn.Module):
    def __init__(self, __C, norm=False, residual=False, base=64, hsize_k=None):
        super(GateSelfAtt, self).__init__()
        self.__C = __C
        self.norm = norm
        self.residual = residual
...
class SGA(nn.Module):
    def __init__(self, __C):
        ...
        self.mhatt1 = Gate_MHAtt(__C, base=64, hsize_k=None)
    ...
    def forward(self, x, y, c, x_mask, y_mask):
        x = self.norm1(x + self.dropout1(
            self.mhatt1(v=x, k=x, q=x, c=c, mask=x_mask)
        ))
