...
from openvgd.loader.position import RegionRelationalEmbedding
...
class DataSet(Data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        rel_img = RegionRelationalEmbedding(bbox) # [n_obj, n_obj, 6]
        rel_img_iter = torch.zeros(100, 100, 6)
        ...
        return torch.from_numpy(frcn_feat_iter),\
               ...
               torch.from_numpy(bbox_abs_iter),\
               rel_img_iter,\
    ...
    def proc_bbox_feat_(self, bbox, img_shape):
    ...
    def proc_bbox_feat(self, bbox, img_shape):
        bbox_feat = np.zeros((bbox.shape[0], 6), dtype=np.float32)
        bbox_feat[:, 0] = ((bbox[:, 0] + bbox[:, 2]) * 0.5) / float(img_shape[1])
        bbox_feat[:, 1] = ((bbox[:, 1] + bbox[:, 3]) * 0.5) / float(img_shape[0])
        bbox_feat[:, 2] = ((bbox[:, 2] - bbox[:, 0]) + 1.) / float(img_shape[1])
        bbox_feat[:, 3] = ((bbox[:, 3] - bbox[:, 1]) + 1.) / float(img_shape[0])
        bbox_feat[:, 4] = bbox_feat[:, 2] / bbox_feat[:, 3]
        bbox_feat[:, 5] = (bbox_feat[:, 2] * bbox_feat[:, 3]) / float(img_shape[0] * img_shape[1])
